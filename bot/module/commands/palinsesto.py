...
async def selezionaCorso(update: Update, context: CallbackContext): #Entry point
    ...
    if corsi.count_documents({}) == 0:
        await update.message.reply_text("Nessun corso disponibile.")
...
async def selezionaGiorno(update: Update, context: CallbackContext):
    ...
    logging.getLogger("gym_bot").info(f"Corso selezionato: {context.user_data['corso']}")
...
async def sendPalinsesto(update: Update, context: CallbackContext):
    ...
    logging.getLogger("gym_bot").info(f"Giorno selezionato: {context.user_data['giorno']}")
    ...
    palinsestoText = ""
    for doc in cursor:
        # logging.getLogger("gym_bot").info(doc)
